...
```{r data_merger, eval = FALSE}

# make csv file from all homes/location csv files
pattern <- paste0(".*omni_.*.csv$")
files <- list.files("../output/omni_clean/", pattern = pattern, full.names = TRUE)
...
write_csv(a, paste0('../output/omni_clean/omni_all_locations_',
                       Sys.Date(), '.csv'))
...
data_all <- read_csv(paste0('../output/omni_clean/omni_all_locations_',
                       export_date, '.csv'))
```
