...
spl_a_diff_lim <- c(-9,9)
 ...
 # make data for plots
a <- rtdata %>%
  select(home, location, datetime, metrics_all) %>%
...
compare_df <- full_join(locations,
...
room.compare <- function(metric, data = compare_df) {
 ...
 a <- ggplot(data, aes(x = hour, y = mean, color = location))+
    geom_line( size = 1)+
