...
# reult from integration
(integrate(kldtest2, 0,300))$value

# result from Osses formula
-0.5*log((x_sd^2)/(e1*ec))

# plot the distributions and the integration of the integral
...
ggplot(data = NULL, aes(x_vals))+
  ...
  geom_line(aes(y= px(x_vals)*log(px(x_vals)/(pxic(x_vals)*pxi(x_vals)))), linetype = 'dashed')
  
kldtest3 <- function(x,y) {
  
  # make sure R does not convert to 0 if result is very small number
  pxi_pxic <- pxi(x)*pxic(y)
...
integrate(inner.integral , 0, 75)

integral1 <- function(x) (integrate(kldtest2, 0,x))$value
integral1(300)
