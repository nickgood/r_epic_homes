...
# test specifitiy formula

pctest <- function(x) dnorm(x, mean=mc, sd=sqrt(ec)) 

kldtest2 <- function(x) {

  # make sure R does not convert to 0 if result is very small number
  p_pc <- ptest(x)*pctest(x)
  
  p_pc <- ifelse( abs(p_pc) > .Machine$double.xmin,
                  p_pc,
                             .Machine$double.xmin)
  
  
          fit_ratio_value <- qtest(x)/p_pc
          ...
          qtest(x)*log(fit_ratio_value)
}

x_vals <- seq(1, 100)

ggplot(data = NULL, aes(x_vals))+
  geom_line(aes(y = ptest(x_vals)))+
  geom_line(aes(y= qtest(x_vals)))+
  geom_line(aes(y= pctest(x_vals)))+
  geom_line(aes(y= pctest(x_vals)*ptest(x_vals)))
...
(integrate(kldtest2, 0,200))$value
