...
# calculate one-day kld values for all possible cust/home/location combos for one metric
kld_max <- map(clusters_all, function(x_cluster) {
...
b <- flatten(a)
c <- bind_rows(b)
d <- c %>%select( -c(low_val, high_val, coeff)) %>%
  select(-everything(), kld_avg, kld_sd, everything())

e <- kld_max%>%select( -c(low_val, high_val, coeff)) %>%
  select(-everything(), kld_avg, kld_sd, everything())
f <- bind_rows(e,d)

write_csv(f, paste0('./csv_created/representativeness_data/kld_max_',
                    Sys.Date(), '.csv'))
```
...
## test alternative method of kld assuming normal distribution
```{r}
m0 <- 30
e0 <- 2^2
