...
date_ranges_entire <- list(c('2020-11-01','2021-04-08'))
home_num <- '004'
...
kld.max <- function(
  ...
  # date range of entire monitoring period in the form:
  # list(c('YYYY-MM-DD', 'YYYY-MM-DD'), c('YYYY-MM-DD', 'YYYY-MM-DD'))
  ...
  scaled.entropy.season <- function(monitor_season) {
    ...
    entropy_data_list <- lapply(
      ...
      function (
        ...
        a <- tibble(
          'coeff' = kld_avg, # mean scaled entropy value
          # # values for sd_int assuming kld_avg_min is a set number
          'high_val' = (kld_avg+kld_sd),
          'low_val' = (kld_avg-kld_sd),
...
# write csv ----------------
# write_csv(kld_max_df, paste0('./csv_created/representativeness_data/kld_max_',
#                     Sys.Date(), '.csv'))
...
kld_max_df <- read_csv('./csv_created/representativeness_data/kld_max.csv')
a <- kld_max_df %>% filter(!is.na(kld_avg))

ggplot(a, aes(y = kld_avg))+
...
# 
# write_csv(a, file =
#             paste0('./csv_created/representativeness_data/rep_data_shape_combine_', Sys.Date(), '.csv'))
...
map(homes_all, function (x_home) {
  map(locations_indoor, function (x_location) {
    entropy.plot(hm = x_home, loc = x_location)
...
all_time <- TRUE
  ...
  if(metric %in% c('pm25', 'voc')) {
    ...
    entire <- entire %>%
       rstatix::identify_outliers(all_of(metric)) %>%
      full_join(entire, by = c('datetime', metric)) %>%
      mutate_at(vars(is.outlier, is.extreme), function(x) {
   ...
   filter(is.extreme != TRUE)
    ...
    entire_fit <- fitdistrplus::fitdist(pull(entire, metric),
        ...
        entire_fit_wieb <- fitdistrplus::fitdist(pull(entire, metric),
                                        "weibull", method = 'mle')
        
    entire_fit_gam <- fitdistrplus::fitdist(pull(entire, metric),
                                        "gamma", method = 'mle')
        # test different distributions fits ----------------

        
    plot.legend <- c("lnorm", "wieb", "gam")

    fitdistrplus::denscomp(list(entire_fit,
              entire_fit_wieb,
              entire_fit_gam), legendtext = plot.legend)
   
    
    fitdistrplus::qqcomp(list(entire_fit,
               entire_fit_wieb,
               entire_fit_gam), legendtext = plot.legend)
    fitdistrplus::cdfcomp(list(entire_fit,
                entire_fit_wieb,
                entire_fit_gam), legendtext = plot.legend)
    fitdistrplus::ppcomp(list(entire_fit,
               entire_fit_wieb,
               entire_fit_gam), legendtext = plot.legend)
...
```{r function_shape_indivdual_kld_max}
