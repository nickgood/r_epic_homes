...
rms_garage <- lapply(c(2,8,5), function(x) get(paste0('rms_', x)))


#### make lists of metric pairs to correlate
    met_pairs_proxy <- list(c('co2', 'voc'), c('co2', 'lux'), c('co2', 'spl_a'))
...
inter_room_breaks <- c('kitchen-bedroom', 'kitchen-living',
                       ...
                       'kitchen-garage', 'bedroom-garage', 'outdoor-garage')
...
inter_metric_breaks <- c('co2-voc', 'co2-lux', 'co2-spl_a', 'pm25-voc', 'pm25-co2')

 inter_metric_labels <- c(expression('CO'[2]*'-TVOC'),
                          expression('CO'[2]*'-Light'),
                          expression('CO'[2]*'-Noise'),
...
# data <- omni_hourly
# hm <- '008'
# rm_pair <- c('kitchen', 'garage')
# rm_pair2 <- c('bedroom', 'garage')
...
#  # test funct
# test <- room.cor.hourly('008', c('bedroom', 'garage'),
...
bind_rows() %>%
 ...
 #    hm = '008',
 #    rms_list = list(c('bedroom', 'garage')),
 #    met = 'pm25',
 ...
 room.hourly.plot <- function (data, met) {
   ...
   ggplot(a, aes(x = hour)) + 
     ...
     theme(
       ...
       strip.text = element_text(size = small-1),
       title = element_text(size = small+1),
