...
# average duplicates from pm10_est addition------------

# from april 7 to april 15 there are some identical values
# just take average


test <- omni_data_clean %>% 
  group_by_at(vars(-pm10)) %>% 
  filter( n()>1) %>%
  ungroup() 
##separate out data in time range
##delete duplicates
## put the singles back in
 a <- omni_data_clean %>%
...
stop('duplicates outside of daylight savings time exist')}
 ...
 test <- omni_data %>%
   filter(home == '007', location == 'kitchen')
 
 test2 <- read_rds('./csv_created/omni_raw_2021-07-14/003_outdoor.rds')
# method 2---------
...
# make datafrae of corrrection factors
