---
title: "Shape - Representativeness"
output:
  html_document:
    toc: true
    toc_float: true
...
# start <- Sys.time()
# 
test<- entropy.table.shape('009', 'pm25',
                           overlap = FALSE,
                           date_ranges_entire = list(
                             c('2020-09-26', '2020-10-22')),
                           tested_sample_sequence = c(1,11,27),
  ...
  map(c('pm25', 'voc'), function(x_metric) {
    map(month.abb[1:12], function(x_cluster) {
      map(locations_indoor, function(x_location){
        get(paste(x_metric, x_location, x_cluster, 'rep_data_shape_pdf', sep = '_'))
      }) %>% bind_rows()
    }) %>% bind_rows()
  }) %>% bind_rows()
...
write_csv(entropy_shape_pdf_df, file =
            paste0('./csv_created/representativeness_data/rep_data_shape_hourly_pdf',
