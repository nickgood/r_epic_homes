...
kld_max_conditional <- kld_max_df_clean %>%
  group_by(sample_length, home, location, energy_cluster, metric, method) %>%
      summarise(max_kld_cutoff = max(kld), .groups = 'drop') %>%
  # summarise(max_kld_cutoff = median(kld), .groups = 'drop') %>%
  rename(sample_min = sample_length)
...
avg_pooled_kld_max_conditional <-  kld_max_df_clean %>%
  group_by(home, energy_cluster, metric, method) %>%
    summarise(max_kld = max(kld), .groups = 'drop')
...
# cor.plot('scaled_kld_metric', 'variance_diff')
...
### Pooled Homes
```{r heatmap_deviations_vs_rep_pooled_homes, fig.height=7, fig.width = 10.5, eval = TRUE}
...
b <- scaled_entropy_df %>% filter(
  ...
  filter(metric == met)
    ...
    geom_violin(data = b, aes(group = sample_length, x = sample_length,
                       y = get(dependent)/coeff), color = 'grey')+
