...
# omit home 011 in shoulder season for severe lack of data
  filter(!(home =='011' & location == 'bedroom' & energy_cluster == 'shoulder')) %>%
    mutate(
      ...
      mean_diff = abs(mean_sample - mean_entire),
...
```{r function_rep_deviation_define}
# testing---------------------
...
ext_outlier_count <- b %>%
  group_by(home, sample_length) %>%
  summarize(ext_outlier_n = sum(ext_outlier == TRUE)/n())
...
coeff <- if(coeff == 'auto') {
  select(b, sym(paste0(dependent,'_sample'))) %>% max()
  } else coeff
...
```{r correlation_no_seasons, eval = FALSE}
# for one condition
# a <- entropy_df_clean %>% filter(method == 'shape', home == '001',
#                                  location == 'living', metric == 'pm25',
#                                  energy_cluster == 'heat')
