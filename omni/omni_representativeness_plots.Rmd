...
# test_time <- entropy_df %>% filter(method == 'time')
# test_old_time <- read_csv('./csv_created/representativeness_data/rep_data_time_hourly_pdf_2021-08-08.csv')
# test3 <- bind_cols(test %>% select('kld_old' = kld), test2 %>% select('kld_new' = kld))
# 
# a <- test_time %>% filter(is.na(kld))
# # %>% group_by(home, location) %>% summarise(n = n())
# b <- test_old_time %>% filter(is.na(kld))
# # %>% group_by(home, location) %>% summarise(n = n())
# c <- bind_cols(a, b[-c(1,2),])
# all.equal(a, b[-c(1,2),])
...
# omit home 011 in shoulder season for severe lack of data
  filter(!(home =='011' & location == 'bedroom' & energy_cluster == 'shoulder')) %>%
    mutate(
      median_diff = abs(median_sample - median_entire),
      ...
      max_diff = abs(max_sample - max_entire),
 ...
 * If "universal" maximum relative entropy is used to scale, the mean of the maximum of the average relative entropy values for sample length of 1 day **between all sensors, energy clusters, and metrics** was chosen as the "zero" point in the scale, for each method separately.
 * If "metric" maximum relative entropy is used to scale, the mean of the maximum of the average relative entropy values for sample length of 1 day **for each metric recorded by all sensors** was chosen as the "zero" point in the scale, for each method separately. 
...
scaled_entropy_df <- entropy_df_clean %>%
  ...
  left_join(
    kld_max_df_clean %>%
      filter(sample_length == 1) %>%
  group_by(home, location, energy_cluster, metric, method) %>%
      summarise(max_kld_cutoff = mean(kld), .groups = 'drop') %>%
  # summarise(max_kld_cutoff = median(kld), .groups = 'drop') %>%
  rename(sample_min = sample_length),
...
pooled_avg_scaled_entropy_df <- scaled_entropy_df %>%
  ...
  summarise(avg_scaled_kld = mean(scaled_kld),
            ...
            n_samples = n(),
            .groups = 'drop')%>%
    group_by(metric, home, energy_cluster, method) %>%
...
avg_scaled_entropy_df <- scaled_entropy_df %>%
  group_by(metric, home, energy_cluster, sample_length, method, location) %>%
  summarise(avg_scaled_kld = mean(scaled_kld),
            high_val = avg_scaled_kld+ sd(scaled_kld),
                      low_val = avg_scaled_kld- sd(scaled_kld),
                        n_samples = n(),
...
entropy.plot <- function (
  ...
  data <- if(room == 'pooled') pooled_avg_scaled_entropy_df else{
...
entropy.plot('012', meth = 'time', room = 'pooled')
...
map(homes_all, function (x_home) {
    entropy.plot(hm = x_home, simple = TRUE, meth = meth, room = 'pooled')
}) %>%
...
* arranged by tier
```{r grid_plot_avg_entropy_tiers, fig.height=4, fig.width = 10.5}
# time-------------------
meth <- 'time'
...
map(homes_all, function (x_home) {
    entropy.plot(hm = x_home, simple = TRUE, meth = meth, room = x_location)
}) %>%
  grid.plots(title = 'Time-Structured Scaled Relative Entropy')%>%
  annotate_figure(
    fig.lab = paste0('Scale Method: ',
...
* arranged by tier
```{r grid_plot_avg_entropy_tiers_rooms, fig.height=4, fig.width = 10.5}
# time-------------------
meth <- 'time'
...
## Compare KLD/Representativeness to other values
...
variables_independent <- c(
  'median_diff', 'median_ratio', 'variance_ratio',
  'max_hour_diff', 'max_diff', 'max_ratio'
                           )
variables_dependent <- c('kld', 'scaled_kld')

# for all conditions
cor.table <- function(pooled){
    map(variables_dependent, function(dependent){
  map(variables_independent, function(independent){
    # if pooled is all, don't separate by any condition
    if(pooled == 'all'){
        a <- scaled_entropy_df 
        test <- cor.test(a %>% pull(dependent),
                         a %>% pull(independent), method = 'spearman')%>%
      suppressWarnings()
        
      list('coeff' = test$estimate, 'p_val' = test$p.value,
     'dependent' = dependent, 'independent' = independent,
     'home' = NA, 'location' = NA,
     'metric' = NA, 'energy_cluster' = NA, 'pooled' = pooled)
    } else if (pooled == 'none') {
...
map(metrics_all, function(x_metric){
    ...
    list('coeff' = NA, 'p_val' = NA,
     'dependent' = dependent, 'independent' = independent, 'home' = x_home, 'location' = x_location,
     'metric' = x_metric, 'energy_cluster' = x_cluster, 'pooled' = pooled)
  ...
  if (nrow(a)>3) {
    test <- cor.test(a %>% pull(dependent), a %>% pull(independent), method = 'spearman')%>%
      suppressWarnings()
    # %>%
    #   tryCatch(error = function(e) result_error)
...
cor_table <- bind_rows(cor.table('none'), cor.table('all'))

# function to plot correlations for given pooling method
cor.plot <- function(var_indep, var_dep, pool_method){
a <- cor_table %>%
  filter(independent == var_indep, dependent == var_dep,
         pooled == pool_method, !is.na(coeff))

ggplot(data = a)+
  geom_histogram(aes(x = coeff))+
  ggtitle(label = paste('Correlation, Pool Method:', pool_method))
}

cor.plot('variance_ratio', 'scaled_kld')

```

### Compare to KLD
```{r plot_deviations_vs_kld}
# time -------------------
meth <- 'time'

{
  ...
  a <- entropy_df_clean %>% filter(metric == met, method == meth,
                                    sample_length %in% (
                                      if(met == 'pm25') pdf_days_pm else
                                        if(met == 'voc') pdf_days_voc else
                                          stop('met not correct')
    ...
    mutate(hour_diff = max_hour_sample - max_hour_entire,
           )
  
  ggplot(a, aes(x = kld, y = hour_diff))+
    geom_bin2d(bins = c(1000,24))+
    xlab("Relative Entropy")+
    ggtitle(label = paste(
        if(meth == 'shape') 'Relative Entropy of', met))+
    coord_cartesian(
      xlim = c(0,0.4),
      ylim = c(-10,10) #median value to...
    )+ 
    # labs(fill = 'Length of Sample, days')+
    # scale_fill_manual( values = length_colors,
    #                    labels=labels) +
...

{
  ...
  a <- entropy_df_clean %>% filter(metric == met, method == meth,
    ...
    mutate(median_diff = median_sample - median_entire,
           )
  
  ggplot(a, aes(x = kld, y = median_diff))+
    geom_bin2d(bins = c(1000,1000))+
    xlab("Relative Entropy")+
    ggtitle(label = paste(
        if(meth == 'shape') 'Relative Entropy of', met))+
    coord_cartesian(
      xlim = c(0,1.75),
      ylim = c(-10,5) #median value to...
    )+ 
    # labs(fill = 'Length of Sample, days')+
    # scale_fill_manual( values = length_colors,
    #                    labels=labels) +
...
### compare to scaled value (Representativeness)
```{r plot_deviations_vs_rep}
# time -------------------
meth <- 'time'

{
  ...
  ggplot(a, aes(x = scaled_kld, y = hour_diff))+
    geom_bin2d(bins = c(1000,24))+
    xlab("Temporal Representativeness")+
    ggtitle(label = paste(
      if(meth == 'time') 'Time-Structured Representativeness of' else
        if(meth == 'shape') 'Magnitude-Based Representativeness of', met))+
    coord_cartesian(
      xlim = c(0,1),# limit representativeness from 0 to 1
      # ylim = c(0,100) #median value to...
    )+ 
    # labs(fill = 'Length of Sample, days')+
    # scale_fill_manual( values = length_colors,
    #                    labels=labels) +
...

{
  ...
  ggplot(a, aes(x = scaled_kld, y = median_diff))+
    geom_bin2d(bins = c(1000,24))+
    xlab("Temporal Representativeness")+
    ggtitle(label = paste(
      if(meth == 'time') 'Time-Structured Representativeness of' else
