...
# omit home 011 in shoulder season for severe lack of data
  filter(!(home =='011' & location == 'bedroom' & energy_cluster == 'shoulder')) %>%
    mutate(
      ...
      variance_diff = abs(variance_sample-variance_entire),
...
kld_max_metric <- kld_max_df_clean %>%
  group_by(sample_length, home, location, energy_cluster, metric, method) %>%
      summarise(cutoff = mean(kld), .groups = 'drop') %>%
  group_by(sample_length, metric, method) %>%
        summarise(max_kld_cutoff = mean(cutoff), .groups = 'drop') %>%
...
kld_max_home <- kld_max_df_clean %>%
  group_by(sample_length, home, location, energy_cluster, metric, method) %>%
      summarise(cutoff = mean(kld), .groups = 'drop') %>%
  group_by(sample_length, metric, method, home) %>%
        summarise(max_kld_cutoff = mean(cutoff), .groups = 'drop') %>%
    # summarise(max_kld_cutoff = median(kld), .groups = 'drop') %>%
  rename(sample_min = sample_length)
...
test <- kld_max_conditional %>%
  group_by(home, location, metric, method) %>%
  summarise(n = n())
```
...
```{r barchart_threshold, eval = FALSE, fig.height=7, fig.width = 10.5}
...
avg_pooled_kld_max_universal <-  kld_max_df_clean %>%
  group_by(home, energy_cluster, metric, method) %>%
    summarise(kld_cutoff = mean(kld), .groups = 'drop')%>%
    group_by(method) %>%
    summarise(max_kld = mean(kld_cutoff), .groups = 'drop')
...
avg_kld_max_universal <-  kld_max_df_clean %>%
  group_by(home, energy_cluster, metric, method, location) %>%
    summarise(kld_cutoff = mean(kld), .groups = 'drop')%>%
...
avg_kld_max_metric <-  kld_max_df_clean %>%
  group_by(home, energy_cluster, metric, method, location) %>%
    summarise(kld_cutoff = mean(kld), .groups = 'drop')%>%
    group_by(method, metric, location) %>%
    summarise(max_kld = mean(kld_cutoff), .groups = 'drop')
...
avg_kld_max_home <-  kld_max_df_clean %>%
  group_by(home, energy_cluster, metric, method, location) %>%
    summarise(kld_cutoff = mean(kld), .groups = 'drop')%>%
    group_by(method, metric, home, location) %>%
    summarise(max_kld = mean(kld_cutoff), .groups = 'drop')
...
avg_pooled_kld_df %>%
 ...
 # rooms not pooled ----------------- 
  ...
  summarize(
    avg_kld = mean(kld), n_samples = n(), .groups = 'drop')%>%
    group_by(metric, home, energy_cluster, method, location) %>%
  mutate(n_samples_max = max(n_samples),
         n_samples_frac = n_samples/n_samples_max) %>%
  ungroup()
...
avg_kld_df %>%
    left_join(avg_kld_max_home,
              by = c('method', 'metric', 'home', 'location')) %>%
  mutate(avg_kld_scaled_home = 1-avg_kld/max_kld)%>%
  select(-max_kld),
...
entropy.plot('012', meth = 'time', scale_method = 'conditional'
             ,room = 'pooled')
...
variables_dependent <- c(
  'median_diff', 'variance_diff',
  'max_hour_diff', 'max_diff'
                           )
# variables_independent <- c('kld', 'scaled_kld_universal',
#                            'scaled_kld_metric',
#                            'scaled_kld_conditional')
...
#       if(pooled == 'all'){
#         a <- scaled_entropy_df %>% filter(kld_max_method == 'universal')
...
#              'home' = NA, 'location' = NA,
#              'metric' = NA, 'energy_cluster' = NA, 'kld_max_method' = max_method,
#              'pooled' = pooled)
#         
#       } else if (pooled == 'none') {
...
cor_table <- bind_rows(cor.table('all'),
                       cor.table('metric'),
                       cor.table('none')
                       )
...
```{r heatmap_deviations_vs_rep, fig.height=7, fig.width = 10.5, eval = TRUE}
...
```{r plot_deviation_vs_sample_length, fig.height=7, fig.width = 10.5}
