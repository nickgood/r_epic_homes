...
kld_max_df_clean <- kld_max_df%>% 
  filter(!is.na(kld), # with insufficient data in cluster/year/no cluster
...
scaled_entropy_df_time <- scaled_entropy_df %>% filter(method == 'time')
scaled_entropy_df_shape <- scaled_entropy_df %>% filter(method == 'shape')
...
a <- scaled_entropy_df_time %>% filter(metric == met, sample_length %in% pdf_days_pm)
...

{
  ...
  a <- scaled_entropy_df_time %>% filter(metric == met, sample_length %in% pdf_days_voc)
...
thresh_vector <- c(0.80, 0.90, 0.95)
...
a <- scaled_entropy_df_shape %>% filter(metric == met, sample_length %in% pdf_days_pm)
...

{
  ...
  a <- scaled_entropy_df_shape %>% filter(metric == met, sample_length %in% pdf_days_voc)
...
avg_scaled_entropy_df <- scaled_entropy_df %>%
  group_by(metric, home, location, energy_cluster, sample_length) %>%
