...

{
  ...
  ggplot(a, aes(x = scaled_kld, fill = as.factor(sample_length)))+
    ...
    annotate('text', x=thresh_vector[1]-0.02, y = -0.1,
             ...
             angle=90, size=xsmall+1, color = 'red') +
...
a <- threshold_df %>% filter(sample_length %in% c(1,3,7,11, 15, 19))
...
ggplot(a, aes(x = as.factor(sample_length), y = frac_greater,
  ...
  facet_wrap(vars( method, metric))+
  geom_text(aes(y = frac_greater+1), label = '5')
...
color.find <- function(x)  {
  case_when(x == 'heat'~ cluster_colors[1],
            x == 'ac' ~ cluster_colors[2],
            x == 'shoulder' ~ cluster_colors[3],
            TRUE ~ 'black')}
...
ach50 <- if(is_empty(ach50)) NA else if (is.na(ach50)) {
  ...
  scale_fill_manual(values = col)+
      ...
      theme(legend.position = 'none',
          ...
          annotate('text', x = 14, y = 0.25, hjust = 0.5, size = xsmall,
...
```{r threshold_plots_shape, eval = FALSE}
...
threshold.freq.table <- function (threshold) {
  ...
  filter(avg_scaled_kld >= threshold) %>% 
  group_by(home, metric, energy_cluster, method) %>%
...
# apply function to all thresholds in threshold_seq
 a <- lapply(thresh_vector, threshold.freq.table)
 a <- bind_rows(a)
