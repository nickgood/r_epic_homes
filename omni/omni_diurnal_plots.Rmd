...
### Diurnal plot for overall sampling period
```{r}
err_ribbon <- TRUE

# make diurnal plots
    a <- diurnal.table(rtdata)
...
clust.diurn <- function(met, data){
  ...
  geom_point(aes(size = n))+
        ...
        annotate('text', x=Inf, y = Inf, hjust = 1, vjust = 1,
                 label = '*Unclassified Days and Group 1 Homes omitted')+
        scale_fill_manual(
...
clust.diurn('pm25', a%>%filter(energy_cluster == 'heat'))
clust.diurn('pm25', a%>%filter(energy_cluster == 'shoulder'))
clust.diurn('pm25', a%>%filter(energy_cluster == 'ac'))
...
clust.diurn('voc', a%>%filter(energy_cluster == 'heat'))
clust.diurn('voc', a%>%filter(energy_cluster == 'shoulder'))
clust.diurn('voc', a%>%filter(energy_cluster == 'ac'))
```
