...
diurnal.table <- function(data,
  ...
  geom_point(aes(size = n))+
    ...
    pooled_diurnal <- diurnal.table(rtdata)

    plot.diurnal(pooled_diurnal, 'pm25')
    plot.diurnal(pooled_diurnal, 'voc')  
...
a <- time.table(rtdata, 'day', groupers = c('home','location'))%>%
  filter(home %in% homes_no5)
...
```{r monthly_homes, eval = FALSE, fig.height=2.5, fig.width = 12}
...
a <- time.table(rtdata, 'month', groupers = c('home','location'))%>%
  filter(home %in% homes_no5)
...
room.compare <- function(metric, data = rtdata,
                         simple = TRUE) {
...
a <- rtdata %>% filter( home %in% homes_no5)
