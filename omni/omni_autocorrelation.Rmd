...
```{r stationarity, fig.width=8, fig.height=4.5}
  ...
  plot <- ggplot(df_acf, aes(x=lag, y=acf)) +
    ...
    {if(simple == TRUE) {
      theme(axis.title = element_blank(),
          ...
          panel.grid.major.x = element_blank(),
          panel.border = element_blank(),
          plot.title = element_text(hjust = 0.5, size = xsmall),
...
lapply('002', ggacf,
       data = omni_daily_data %>% filter(date < ymd('2020-08-13')),
...
a <- acf_lags %>% filter(days_avail < days_avail_min) %>% nrow()/n_metrics_lags
b <- acf_lags %>% filter(missing_pct > missing_pct_min) %>% nrow()/n_metrics_lags

```

### omitted `r a` monitoring periods for each metric
### because less than `r days_avail_min` available days  

### omitted `r b` monitoring periods for each metric
...
ggplot(data = a , aes(x = location, y = lag_signif))+
  ...
  scale_y_continuous(oob= scales::squish_infinite)+
            scale_color_identity(name = "Insignifiance reached within 30 days",
                               breaks = c("black", "red"),
                               labels = c("Yes", "No"),
                               guide = "legend")+
  labs(tag = paste0('*need to add in ', inf_count, ' points\n',
                    'that did not reach insignificance.'))+
