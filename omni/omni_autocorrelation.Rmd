...
# # filter data to one home/location and arrange by date
#           avg_data <- data %>%
#   filter(home == hm & location == loc) %>%
#                 filter(datetime > ymd_hms(start_date) & datetime < ymd_hms(end_date)) %>%
#   mutate(date = floor_date(datetime, unit = 'days')) %>%
...
# # all homes/locations in one dataframe, hourly
...
# lags <- c(1:lag.max)
# 
# a <- map(lags, function(x) {
#   avg_data_all %>%
#               filter(home == '009' & location == 'garage')%>%
#   pull(all_of(metric)) %>%
#   diff(lag = x) %>%
#     is.na() %>%
...
#plot
  ...
  lag_signif <- ifelse(length(a)>0, min(a), Inf) %>%
    ...
    mutate(home = hm, location = loc, metric = metric,
           days_avail = days_avail, missing_pct = missing)
...
lapply(homes_all, function(x) {
  ggacf(hm = x,
        data = omni_daily_data,
        loc = 'living', lag_max_plot = 14, metric = 'voc'
  )[['plot']]
}
)
