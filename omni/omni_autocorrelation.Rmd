...
# convert any NA clusters to "unclasssified"
...
a <- acf_lags_no_inf %>% 
  filter(energy_cluster %in% c('ac', 'shoulder', 'heat', 'all')) %>%
  filter(metric == met)
...
###### clusters
  # convert values that were infinite to above the maximum tested value
acf_lags_cluster_no_inf <- acf_lags_cluster %>%
  mutate(lag_signif = if_else(is.infinite(lag_signif), 35, lag_signif))


a <- acf_lags_cluster_no_inf %>% 
