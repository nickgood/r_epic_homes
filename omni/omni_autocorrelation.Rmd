...
ggacf <- function(data,  hm, loc, lag_max_plot, metric,
  ...
  plot <- ggplot(df_acf, aes(x=lag, y=acf)) +
    ...
    coord_cartesian(xlim=c(0, lag_max_plot), ylim = c(-0.25,1))+
    {if(simple == TRUE) {
...
```{r ggacf_grid_plots_no_clusters, fig.width=8, fig.height=10.5}
...
metric <- 'pm25'
  ...
  acf.grid(title = paste(loc, metric, 'no clusters'))
