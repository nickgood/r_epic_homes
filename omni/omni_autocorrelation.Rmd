...
# define homes that have data for all indoor rooms
...
energy_cluster_df<- tribble(~home, ~type, ~start_date, ~end_date,
                     ...
                     '004', 'none', '2020-08-10', '2020-10-12',
                     ...
                     '010', 'heat_1', '2020-08-01', '2020-10-18',
                     '010', 'none', '2020-10-19', '2020-12-01',
                     '010', 'heat_2', '2020-12-02', '2021-05-23',
...
between(a$date2[1], ymd('2020-07-20'), ymd('2020-07-22'))
between(ymd('2020-07-20'), ymd('2020-07-20'), ymd('2020-07-22'))
...
for(i in 1:length(homes_clustered)) { 
  # for each home
  # identify the df matrix index of each cluster type
  cluster_type_indeces <- which(energy_cluster_df$home == homes_clustered[i])
    ...
    ] <- energy_cluster_df[cluster_type_indeces[j], 'type'][[1]]
  }
}
...
```{r functions_autocorrelation, include= FALSE}
...
ggacf <- function(data,  hm, loc, lag_max_plot, metric,
  ...
  plot <- ggplot(df_acf, aes(x=lag, y=acf)) +
    ...
    theme_bw() +
    xlab("Lag") +
    ggtitle(if_else(simple == TRUE, "", paste(hm, loc, metric, cluster))) +
    annotate( 'text', x = Inf, y = Inf, hjust = 1, vjust = 1,
              label = paste("*missing", signif(missing,3), "% of daily values\n",
                            "in home sampling period"), size = small)+
    coord_cartesian(xlim=c(0, lag_max_plot), ylim = c(-1,1))+
...
lapply(homes_all, ggacf,
       data = omni_daily_data, loc = 'living', lag.max = 48, metric = 'pm25')
...
# plot results for one location and metric
lapply(homes_avail, function(x) {
  ggacf(hm = x,
        data = omni_daily_data,
        loc = 'living', lag_max_plot = 14, metric = 'pm25',
  )[['plot']]
}
)


# return data for all homes, locations, and metrics
acf_lags <- 
...
# one month
  lapply(homes_all, ggacf,
         data = omni_daily_data %>%
           filter(date> ymd('2020-11-15') & date <ymd('2020-12-15')),
         loc = 'living', lag_max_plot = 14, metric = 'pm25')
...
```{r lag_dependence_n}
# test hoe dependent the significant lag value seems to be on n

  # convert values that were infinite to above the maximum tested value
acf_lags_no_inf <- acf_lags %>%
  mutate(lag_signif = if_else(is.infinite(lag_signif), 35, lag_signif))

# look at possible effects of sample size
ggplot(data = acf_lags_no_inf , aes(x = days_avail, y = lag_signif))+
  geom_point(aes(color = location))+
  facet_wrap(vars(metric))

# boxplots by location
ggplot(data = acf_lags_no_inf , aes(x = location, y = lag_signif))+
  geom_boxplot()+
  facet_wrap(vars(metric))
