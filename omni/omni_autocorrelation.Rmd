...
all_homes <- home.list(c(1:4, 7:16))
...
# see which homes/locations do not have missing daily average values
 sapply(all_homes, count.missing,
        loc = 'living',
        data = omni_data,
       metric = 'pm25')
...
#plot
  plot <- ggplot(df_acf, aes(x=lag, y=acf)) +
    ...
    theme_bw() +
    xlab("Lag") +
    ggtitle(if_else(simple == TRUE, "", paste(hm, loc))) +
...
# all dates
# return data
acf_lags <- lapply(all_homes, function(x) {
  ggacf(hm = x,
        data = omni_daily_data,
...
acf.plot.nomiss <- function (hm, rm,
  ...
  date_col <-
    seq(ymd_hms(start_date), ymd_hms(end_date),
        dhours(1))%>%
    as.data.frame() %>%
    rename('date' = '.')
