...
a <- entropy_time %>% filter(sample_length %in% c(1,3) &
                               !is.na(rep) &
                               metric == x_metric & 
                               home %in% homes_tier3)
...
p1 <- ggplot(a[['ac']])+
  ...
  ggtitle(paste0(x_metric, ': Samples from GROUP 3 homes and rooms pooled'))+
    ...
    theme(
      ...
      legend.title = element_text(size = large),
...
p1 <- ggplot(a[['ac']])+
  ...
  #           y = 0.75, size = xsmall)+
  facet_wrap(vars(energy_cluster))+
  geom_vline(xintercept = thresh_vector[1], linetype = 'dashed', size = 1)+
  xlab('Representativeness')+
  ggtitle(paste0(x_metric, ': Samples from GROUP 3 homes and rooms pooled'))+
      # labs(fill = 'Length of Sample, days')+
      labs(fill = 'Sample Length, days')+
  ...
  group_by(energy_cluster, sample_length) %>%
    mutate(n = n(),
         ...
         # above2 = signif( sum(rep>thresh_vector[2])/n*100, 2),
                  ...
                  label = paste0( sample_length, " (", above1, "%)")
...
p1 <- ggplot(a[['ac']])+
  ...
  ggtitle(paste0(x_metric, ': Samples from GROUP 3 homes and rooms pooled'))+
    ...
    theme(
      ...
      axis.ticks.y = element_blank(),
      plot.title = element_blank(),
      strip.text = element_text(size = large+8),
      axis.text = element_blank(),
      axis.title = element_blank(),
      legend.text = element_text(size = large+12),
