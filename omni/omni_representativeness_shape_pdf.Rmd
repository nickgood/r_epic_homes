...
# date_ranges_entire <- list(c('2020-10-13','2020-11-08'))
# tested_sample_sequence <- seq(7,19,21)
# home_num <- '013'
  ...
  scaled.entropy.season <- function(monitor_season) {
    ...
    kld.period <- function(days) { # days = number of days in short samping period
      ...
      if(length(all_dates_season) <= days*24) {
        warn <<- 'short_season'
        a <- rep(NA, 2) %>% as.integer() %>% as.data.frame()
      ...
      a <- lapply(colnames(a), function(x) {
        ...
        mean_sample <- a[1]
      variance_sample <- a[2]
      # calculate the difference between the sample and entire period
      # means and variances
      diff_mean <- mean_sample - mean_entire
      diff_variance <- variance_sample - variance_entire
      
      # find kld value
      # from formula from Osses paper
      sample_kld <- 0.5*(variance_sample/variance_entire-1-log(variance_sample/variance_entire)+
...
map(a, function(x) {
            list(
              'kld' = x[['kld']],
              'diff_mean' = x[['diff_mean']],
              'diff_variance' = x[['diff_variance']],
...
test<- entropy.table.shape('013', 'pm25',

                           date_ranges_entire = list(
                             c('2020-10-18', '2020-11-08')),
                           tested_sample_sequence = c(7,11,23,27),
                           location_type = 'living',
                           all_time = TRUE)
end <- Sys.time()
run2 <- end- start
