...
#                      suppressWarnings() %>% # warnings not a problem in this case
#                      tryCatch(
#                        error = function(e)   { # if error because log dist cannot be fit
#                          list('estimate' = c('meanlog' = NA,
#                                              'sdlog' = NA) )
#                        }
#                      ) 
#                    
#                    if(length(sample_fit$estimate) == 2) {
#                    a <- c(sample_fit$estimate['meanlog'], sample_fit$estimate['sdlog'])
#                    } else {
#  a <- 'error'
# 
...
#                  # return values for one running window
#                  a <- as.data.frame(a)
#                  colnames(a) <- paste(as.character(as.POSIXct(y[1],
#                                                         origin = '1970-01-01')),
#                                       as.character(as.POSIXct(y[length(y)],
#                                                         origin = '1970-01-01')),
#                                       sep = ' - ')# give name to column just to suppress "new name" message
#                  }
#                  a
#                }
#         )
...
# 
# 
# # test function for all dates saparately, see where error is
# test <- lapply(seq(days*24*12, length(all_dates_season), 24*12), test.fun) 
# 
# # error occurs at the 24th index, so test that sample
# day_num <- 23
# y <- all_dates_season[(day_num*24*12):((day_num+days)*24*12)]
# 
# 
# sample <- data_season %>%
#   filter( location == location_type & datetime %in% y)
...
#   suppressWarnings() 
# 
# ggplot(data = sample, aes(x = get(metric)))+
#   geom_histogram()
# 
# 
# a <- omni_data %>% filter(home == '010',
#                           datetime >=ymd ('2020-12-18'),
#                           datetime <ymd ('2020-12-27'))
# ggplot(data = a, aes(x = datetime, y = get(metric)))+
#   geom_line()+
