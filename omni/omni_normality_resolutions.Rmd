...
```{r csv_censored_creation, eval = FALSE}

# create data with upper 1 percentile removed
omni_censor_high <- omni_data %>%
  select(-c(timestamp, filename, pm10_est, score, lux, spl_a))%>%
  pivot_longer(cols = c(pm25, voc, co2), names_to = 'metric',
               values_to = 'value') %>%
  group_by(home, location, metric) %>%
  mutate(percentile99 = quantile(value, 0.99)) %>%
  filter(value < percentile99) %>%
      select(-percentile99) %>%
  ungroup() %>%
  pivot_wider(names_from = metric, values_from = c(value))

omni_hourly_censor_high <- omni_censor_high %>%
...
omni_daily_censor_high <- omni_censor_high %>%
...
# create csvs
write_csv(omni_censor_high, paste0('./csv_created/omni_censor99_',
Sys.Date(),'.csv'))

write_csv(omni_hourly_censor_high, paste0('./csv_created/omni_hourly_censor99_',
Sys.Date(),'.csv'))

write_csv(omni_daily_censor_high, paste0('./csv_created/omni_daily_censor99_',
Sys.Date(),'.csv'))

```

```{r import_censored_data, eval = FALSE}
omni_censor_high <- read_csv('./csv_created/omni_censor99.csv')
omni_hourly_censor_high <- read_csv('./csv_created/omni_hourly_censor99.csv')
omni_daily_censor_high <- read_csv('./csv_created/omni_daily_censor99.csv')
...
```{r plot_normality_diff_resolutions, fig.width=10.5, fig.height=8, eval = FALSE, results= 'asis'}
...
```{r plot_censored_normality_diff_resolutions, fig.width=10.5, fig.height=8, eval = FALSE, results='asis'}
# make qqplots for all homes/locations/metrics
# censored data
...
* Home 010 not tested for lack of good fit?
...
```{r pm25_best_dist, fig.width=10.5, fig.height=4, eval = TRUE}
...
plot<- ggplot(dist_compare_df, aes(x = sensor, y = aic, fill = dist_type, color = min))+
...
a <- dist_compare_df %>%
  ...
  group_by(dist_type) %>%
  summarise(n_min = n()) %>%
  mutate(pct_min = n_min/sum(n_min), resolution = res)
}
plot
...
```{r voc_best_dist, fig.width=10.5, fig.height=4, eval = TRUE}
