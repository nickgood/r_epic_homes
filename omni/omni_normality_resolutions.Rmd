...
homes_no10 <- homes_all[-which(homes_all == '010')]
...
```{r plot_normality_diff_resolutions, fig.width=10.5, fig.height=8, eval = FALSE}
...
```{r plot_distribution_tests, fig.width=10.5, fig.height=8}
  ...
  a <- data_metric %>% filter(location ==room)
   ...
   fit_log <- fitdistrplus::fitdist(pull(a, met),
    ...
    plot.legend <- c("lnorm", "weib", "gam")
        ...
        a <- fitdistrplus::denscomp(list(fit_log,
                                fit_weib,
                                fit_gam), legendtext = plot.legend,
                           main = paste('dens', hm, met, room, resolution),
                           plotstyle = 'ggplot')
    
    
   b <-  fitdistrplus::qqcomp(list(fit_log,
                              fit_weib,
                              fit_gam), legendtext = plot.legend,
                         main = paste('qq', hm, met, room, resolution),
                           plotstyle = 'ggplot')

    c <- fitdistrplus::cdfcomp(list(fit_log,
                               fit_weib,
                               fit_gam), legendtext = plot.legend,
                          main = paste('cdf', hm, met, room, resolution),
                           plotstyle = 'ggplot')

   d<-  fitdistrplus::ppcomp(list(fit_log,
                              fit_weib,
                              fit_gam), legendtext = plot.legend,
...
# test <- map(rooms_avail, function(room) {
#     
#   a <- data_metric %>% filter(location ==room)
#     
#    fit_log <- fitdistrplus::fitdist(pull(a, met),
...
distribution.plotter('004', 'pm25', 'day')

```

```{r function_plot_and_df_best_distribution}
...
```{r pm25_best_dist, fig.width=10.5, fig.height=8}
...
res <- 'day'
{
dist_compare_df <- map(homes_no10, function (x_home){
...
```{r voc_best_dist, fig.width=10.5, fig.height=8}
