    ...
    info_gain_rooms <- 
      map(rooms_with_data,
          function(room){
            ...
            # use double the maximum 
                        variance_background <-
                          (2*max(c(
                            sqrt(var(data_wide_complete_log %>% pull(room))),
                            sqrt(var(data_wide_complete_log %>% pull(other_rooms[1]))),
                            sqrt(var(data_wide_complete_log %>% pull(other_rooms[2])))
                          )))^2
            ...
            #         )
              ...
              -0.5*
                  log(
                    det(cov_mat_all)/
                        (det(cov_mat_complement)*variance_background)
                    )
...
write_csv(info_gain_df, file =
            paste0('./csv_created/representativeness_data/info_gain_method2.csv'))
# make dated backup
write_csv(info_gain_df, file =
            paste0('./csv_created/representativeness_data/info_gain_method2_',
                   Sys.Date(), '.csv'))
...
info_gain_df <- read_csv('./csv_created/representativeness_data/info_gain_method2.csv')
