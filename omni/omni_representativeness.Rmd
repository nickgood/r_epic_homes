...
tested_sample_sequence <- sequence
sample_days_min <- 3
home_num <- '010'
location_type <- 'living'
data <- omni_data
monitor_period <- date_ranges_entire[1]
metric <- 'pm25'
days <- 4
days2 <- 4
...
all_data <- data %>%
  ...
  filter(
    datetime >= ymd(monitor_period[1]) &
    datetime <= ymd(monitor_period[2])
  )
  ...
  # y<- all_dates[(3*24*4):((3+days)*24*4)]
   ...
   a <-
    runner(all_dates,
       ...
       at = seq(days*24*12, length(all_dates), 24*12),
...
test<- scaled.entropy.table.shape('007', 'pm25',
                           date_ranges_entire = list(c('2020-12-01', '2020-12-31'),
                                                     c('2020-11-01', '2020-11-30')),
...
# data making functions for shape distributions-----------------------------



# # make dataframes and name in form: "home_009_pm25"
# # so they can be saved separately and then joined
# make.data.shape <- function(hm, metric, month,
#                       tested_sample_sequence) {
#   
#   tryCatch(scaled.entropy.table.shape(hm, metric,
#                                 months_entire = month,
...
# so they can be saved separately and then joined
# for date ranges
make.data.range.shape <- function(hm, metric, ranges,
                      tested_sample_sequence) {
  tryCatch(scaled.entropy.table.shape(hm, metric,
                                date_ranges_entire = ranges,
                                tested_sample_sequence = tested_sample_sequence),
         error = function(e) NULL)

}

a <- lapply(home.list(1:11), make.data.range.shape,
       metric = 'pm25', ranges = dates, tested_sample_sequence = c(4,5,6))

  b <- scaled.entropy.table.shape(home_num = '003',
                           metric = 'pm25',
                        date_ranges_entire = dates,
                            tested_sample_sequence = c(4,5,6))

a <- make.data.range.shape('009', 'pm25', dates, c(4,5,6))

make.data.range.shape <- function(hm, metric, ranges,
                      tested_sample_sequence) {
  
  a <- lapply(ranges, function(x_range) {
         
     tryCatch(scaled.entropy.table.shape(home_num = hm,
                                         metric = metric,
                                date_ranges_entire = x_range,
                                tested_sample_sequence = tested_sample_sequence),
         error = function(e) NULL)
  }
  ) %>%
  ...
  # make a variable name for this data
...
dates <- list(c('2020-12-01','2020-12-31'))

  a2 <- lapply(dates, scaled.entropy.table.shape,
              home_num = '008',
              data = omni_data,
                                         metric = 'pm25',
                                tested_sample_sequence = c(4,5,6),
...
make.data.range.shape('008', 'pm25', dates,
                      sequence)

#apply this data-making function to all homes
rep.data.shape <- function (hms, metric, ranges,
  ...
  hms,
               # loop through for date ranges
               function(hm) {
                 lapply(ranges,
                      make.data.range.shape,
                      hm = hm,
                      metric = metric,
                      tested_sample_sequence = tested_sample_sequence
                      )
...
# make dataframes and name in form: "home_009_pm25"
# so they can be saved separately and then joined
make.data.pdf.shape <- function(hm, metric, month,
                      tested_sample_sequence,
                                n_bins) {
  
  tryCatch(scaled.entropy.pdf.shape(hm, metric,
                                months_entire = month,
                                tested_sample_sequence = tested_sample_sequence,
                                n_bins = n_bins),
         error = function(e) NULL)
...
rep.data.pdf.shape <- function (hm, metric, mnths,
                      tested_sample_sequence = c(1:28),
                                n_bins = 50) {
...
# test <- scaled.entropy.pdf.shape('003', 'pm25_ihs', months_entire = 'Dec',



# for date range
make.data.range.pdf.shape <- function(hm, metric, range,
                      tested_sample_sequence) {
  tryCatch(scaled.entropy.pdf.shape(hm, metric,
...
# # test function
# make.data.range.pdf.shape('003', 'pm25_ihs', season_list, 4)
...
# start <- Sys.time()
# rep.data.shape('003', met, 12,
#                tested_sample_sequence = samp_seq,
#                n_bins = 15)
# end <- Sys.time()
# 
# time <- end-start
# 
# dat15 <- home_003_pm25_ihs
# 
# dat15 <- dat15 %>%
#   mutate(n_bins = 15)

# dat_test <- rbind(dat5, dat10, dat15, dat25, dat35, dat50)


rep.data.shape('001', met, month_seq,
               tested_sample_sequence = samp_seq)

rep.data.shape('002', met, month_seq,
               tested_sample_sequence = samp_seq)
rep.data.shape('003', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('004', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('005', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('006', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('007', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('008', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('009', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('010', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('011', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('012', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('013', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('014', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('015', met, month_seq, 
               tested_sample_sequence = samp_seq)
rep.data.shape('016', met, month_seq, 
               tested_sample_sequence = samp_seq)
...
write_csv(home_data, file =
            paste0('./representativeness_data/rep_data_shape_', met, '_', Sys.Date(), '.csv'))


# read in csv file
entropy_pm25_shape <- read_csv(file = './representativeness_data/rep_data_shape_pm25_ihs.csv')
...
home1 <- make.data.range.shape('001', met, season_list, sequence)
home2 <- make.data.range.shape('002', met, season_list, sequence)

start <- Sys.time()
home3 <- make.data.range.shape('003', met, season_list, sequence)
end <- Sys.time()
time <- end-start

home4 <- make.data.range.shape('004', met, season_list, sequence)
home5 <- make.data.range.shape('005', met, season_list, sequence)
home6 <- make.data.range.shape('006', met, season_list, sequence)
home7 <- make.data.range.shape('007', met, season_list, sequence)
home8 <- make.data.range.shape('008', met, season_list, sequence)
home9 <- make.data.range.shape('009', met, season_list, sequence)
home10 <- make.data.range.shape('010', met, season_list, sequence)
home11 <- make.data.range.shape('011', met, season_list, sequence)
home12 <- make.data.range.shape('012', met, season_list, sequence)
home13 <- make.data.range.shape('013', met, season_list, sequence)
home14 <- make.data.range.shape('014', met, season_list, sequence)
home15 <- make.data.range.shape('015', met, season_list, sequence)
home16 <- make.data.range.shape('016', met, season_list, sequence)
...
# read in csv file
entropy_shape_season_pm25 <- read_csv(file = './representativeness_data/rep_data_shape_season_pm25_ihs.csv')
...
low_vals <- seq(0.0001, 0.01, 0.0001)
low_log_vals <- c(1 %o% 10^(-10:1))
high_vals <- seq(1,5,0.1)
low_high_vals <- c(low_vals, high_vals)
.Machine$double.xmin

integrate.kld <- function(x) {
  
  sapply(x, function(y) {
    temp <- integrate(f = kld_fit, lower = y,
                                  upper = Inf, subdivisions = 10000)
    temp$value
  }
  ) %>% as.data.frame() %>% rename('integral' = '.') %>%bind_cols('pm_val' = x) %>%
    pull('integral')
}

# # test integrate function for multiple inputs
# int <- integrate.kld(c(0.01, 0.001))
...
scaled.entropy.table.shape <- function(home_num, metric,
  ...
  if(metric %in% c('pm25', 'voc')) {
    ...
    pct_zero <- nrow(data_zero)*100/nrow(data)
    ...
    # kld.avg <- function(days) { # days = number of days in short samping period
      ...
      log_fit_sample <- function(x, y) {
        
        fit_value <- dlnorm(y, meanlog = test_estimates[[x]]%>% pull(meanlog),
                            sdlog = test_estimates[[x]]%>% pull(sdlog))
      ...
      # look at samples vs fitted log distribution
      ggplot()+
