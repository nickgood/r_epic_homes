...
entire_fit <- fitdist(pull(entire, metric), "lnorm", method = 'mle') %>%
           suppressWarnings()
...
# filter out only the amount of days specified by k
         ...
         sample_fit <- fitdist(pull(sample, metric), "lnorm", method = 'mle') %>%
           suppressWarnings()
...
rep.data.shape <- function (hm, metric, mnths,
...
# for date range
make.data.range.shape <- function(hm, metric, range,
                      tested_sample_sequence) {
  tryCatch(scaled.entropy.table.shape(hm, metric,
...
a <- sapply(colnames(a), function(x) {
  a %>%
  pull(x) %>%
    KL.plugin(entire_discrete, unit = 'log2')
})
...
a <- sapply(colnames(a), function(x) {
  a %>%
  pull(x) %>%
    KL.plugin(pull(entire, mean), unit = 'log2')
})
...
month_seq <- c(7:12, 1:2)

# vector of sample lengths to make data for (months)
samp_seq <- seq(1:28)

# vector of sample lengths to make data for (ranges)
sequence <- c(1:28)
...
Assumptions
 * KLD calculation assumes KLD is zero when probability distribution of shorter sample approaches zero (from limit value) 
...
## Distribution Shape: Assume Normal Distribution of Logged Values

### Test effects of number of bins on analysis

```{r bin_testing, message=FALSE}

# testing differences between using different bin numbers
# both run time and coefficient results
...
# write_csv(dat_test, 'n_bins_test.csv')

# read in resulting data
a <- read_csv('n_bins_test.csv')

# plot results
# show how bin amounts change calculated coefficient
ggplot(a, aes(x = n_bins, y = coeff))+
  geom_point()+
  facet_wrap(vars(sample_length))

# ggplot(a, aes(x = sample_length, y = coeff))+
#   geom_point()+
#   facet_wrap(vars(n_bins))


# for each sample_length,
# normalize coefficient based on largest coeff values (when 50 bins were used)

# find coefficient when n_bins = 50
b <- a %>%
  filter(n_bins == 50) %>%
  mutate(coeff_50 = coeff) %>%
  dplyr::select(-c(coeff, n_bins))

# scale the original coefficient
c <- a %>%
  left_join(b) %>%
  mutate(coeff_scaled = coeff/coeff_50)

# plot normalized values
ggplot(c, aes(x = n_bins, y = coeff_scaled))+
  geom_point()+
  facet_wrap(vars(sample_length))+
  expand_limits(y = 0)
...
rep.data.shape('003', met, lkjdshflkjsdfh, 
               tested_sample_sequence = samp_seq)
