...
# make column of all dates (increments of 5 min) within year
    ...
    all_dates_year <- seq.POSIXt(from = as.POSIXct( monitor_season[1], tz = 'UTC'),
    ...
    # calculate missingness of data points in the monitoring year
...
# make a vector of dates when value was recorded in season
...
kld.avg <- function(days) { # days = number of days in short samping period
  ...
  # for testing
  # y<- all_dates_season[(3*24*4):((3+days)*24*4)]
...
entropy_data_season <- bind_rows(a) %>%
  mutate(method = 'shape',
         period_compare = if_else(all_time == TRUE, 'year', 'season'))
...
test<- scaled.entropy.table.shape('001', 'pm25',
                           date_ranges_entire = list(c('2020-12-01', '2020-12-31'),
                                                     c('2020-11-01', '2020-11-30')),
...
a <- lapply(homes_all, function(x_home) {
            
  a <- tryCatch(scaled.entropy.table.shape(x_home, metric = met,
                                date_ranges_entire = list(c(pick.start(x_home, clust),
                                                            pick.end(x_home, clust))),
                                tested_sample_sequence = c(4,5,6),
