...
```{r import_omni_15min_data, eval = FALSE}
...
b <- '2020-12-31'
    ...
    c_old <- seq.POSIXt(from = floor_date(
    as.POSIXct( a,
                tz = 'UTC'), unit = 'day'),
    ...
    c_new <- seq.Date(from = floor_date(ymd(a)))
...
scaled.entropy.month <- function(monitor_period) {
    ...
    if(second(as.POSIXct(monitor_period[1])) != 0 |
       ...
       hour(as.POSIXct(monitor_period[2])) != 0) stop('Times cannot be specifed in date range endpoints.')
...
all_data <- data %>%
  ...
  filter(
    date >= ymd(monitor_period[1]) &
    date <= ymd(monitor_period[2])
  )
