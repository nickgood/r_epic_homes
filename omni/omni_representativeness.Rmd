...
a <- lapply(colnames(a), function(x) {
    ...
    a <- a_orig %>% pull(colnames(a_orig[1]))
    ...
    # find kld value
    return(integrate(kld_fit, lower = 0, upper = Inf,
                     stop.on.error = FALSE)$value) # fails because of low values?
    
  }) 

integrate(kld_fit, lower = 0.0000000001, upper = Inf,
                     stop.on.error = FALSE)$value
integrate(kld_fit, lower = .Machine$double.xmin, upper = Inf)$value
pm_vals <- seq(0.)
ggplot(kld_fit())
...
test<- scaled.entropy.table.shape('010', 'pm25',
                           date_ranges_entire = list(c('2020-12-01', '2020-12-31'),
                                                     c('2020-11-01', '2020-11-30')),
