...
```{r import_omni_15min_data}
rtdata<- read_csv('./../../purgiel_r_epic_homes/omni/OMNIdata_clean.csv')
...
a <- '2020-12-01'
ceiling_date(as.POSIXct(a, format="%Y-%m-%d",
                              na = "NA", tz = 'UTC')+1)
ceiling_date(ymd(a)+1)
...
scaled.entropy.month <- function(monitor_period) {
    ...
    if(second(as.POSIXct(monitor_period[1])) != 0 |
       minute(as.POSIXct(monitor_period[1])) != 0 |
       hour(as.POSIXct(monitor_period[1])) != 0|
       
       second(as.POSIXct(monitor_period[2])) != 0 |
       minute(as.POSIXct(monitor_period[2])) != 0 |
       hour(as.POSIXct(monitor_period[2])) != 0) warning('Times specifed in date range endpoints are not acknowledged.')
...
all_data <- data %>%
  ...
  filter(
    date >= floor_date(ymd(monitor_period[1]), unit = 'day'),
    date < ceiling_date(ymd(monitor_period[2]), unit = 'day')
  )
...
# make column of all dates within range
    ...
    as.POSIXct( monitor_period[1],
                tz = 'UTC'), unit = 'day'),
    # round up to next day for the end date
                    to = ceiling_date(
                      as.POSIXct( monitor_period[2],
                                  tz = 'UTC')+1,
