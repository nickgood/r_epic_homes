...
##in a home at a given hour 
 r.coeff.hour <- function(hm, rm_pair,
                          hr, met,
                          method,
                          data) {
   hour_data <-
...
#  # test funct
# test <- r.coeff.hour('008', c('living', 'garage'),
#              23, 'pm25', 'spearman', omni_hourly_data)
 ...
 cor.hourly <- function (hm, rm_pair,
                           met, method,
                           data) {
    
    cor_results <- sapply(c(0:23), r.coeff.hour,
                          hm = hm,
                          met = met,
                          method = method,
                          rm_pair = rm_pair,
...
#  # test funct
# test <- cor.hourly('008', c('living', 'garage'),
...
bind_rows()
   ...
   a <-a %>%
     ...
     ##add scale outdoor temp axis for two y axes

      scale_ratio <- max(a$var_2, na.rm = TRUE)
   ...
   ggplot(a, aes(x = hour)) + 
     ggtitle(hm)+
     geom_point(aes(y = as.numeric(coeff), size = ifelse(p_val < 0.05, 3, 1),
                    color = loc_pair))+
     ...
     geom_point(aes(y = var_2/scale_ratio, shape = 'outdoor T'))+
        scale_y_continuous(
  ...
  ##function to plot in a grid
 room.hourly.plot.grid <- function (hms, rms_list, met,
                                    method = 'spearman',
                                    func, ncolumn = 4,
                                    data = omni_hourly_data
                                       ) {
