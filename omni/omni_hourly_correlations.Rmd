...
homes_all <- home.list(1:17)
    ...
    metrics_all <- c('pm25', 'voc', 'co2', 'temp','rh', 'light', 'noise')
...
# plot
   ggplot(a, aes(x = hour)) + 
     ...
     geom_point(aes(y = var_2/scale_ratio, shape = paste(loc_2, met_2)),
                color = 'dimgray'),
        scale_y_continuous(
...
```{r between_room_hour_correlations_grid, warning=FALSE, fig.width=10.5, fig.height=8,eval = TRUE, results='asis'}
...
#  #          height = 8, width = 10)
    ...
    met_pairs <- list(c('co2', 'voc'), c('co2', 'light'))
...
lapply(locations_indoor, function(x_location){
   ...
   # ggsave(paste0('hourly_corr_',metric,'_', Sys.Date(),'.pdf'), grid,
    ...
    # make plot grid and save for all metrics
lapply(metrics_all, function(x_location){
