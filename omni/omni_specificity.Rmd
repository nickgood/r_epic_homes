...
energy_labels <- c('Cooling', 'Shoulder', 'Heating')
...
# calculate mutual info based on Osses Equation 2
            ...
            variance_room <- var(data_wide_complete_log %>% pull(room))
             ...
             mutual_info <- -0.5*log( det(cov_mat_all)/
                                       (variance_room*det(cov_mat_complement)))
...
write_csv(mutual_info_df, file =
            paste0('./csv_created/representativeness_data/mutual_info.csv'))
# make dated backup
write_csv(mutual_info_df, file =
...
## Boxplot values with seasons colored
```{r plot_spec_rooms, fig.width = 8, fig.height = 3}

spec_df$metric <- factor(spec_df$metric, levels = c('pm25', 'voc', 'co2'))

ggplot(spec_df, aes(x = location, y = specificity))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter( aes(color = energy_cluster, shape = energy_cluster), width = 0.1)+
  facet_wrap(vars(metric), labeller = labeller.all)+
  ...
  theme(
      ...
      axis.text.y = element_text(size = small-4),
...
ggplot(spec_df, aes(x = location, y = specificity))+
 ...
 # specify shape and color for each season
  ...
  theme(
      ...
      axis.text.x = element_text(size = small-1),
      axis.text.y = element_text(size = small-1),
...
```{r table_rank, fig.height = 3.5, fig.width = 8, eval = FALSE}
...
### Seasons Separate
```{r table_rank_season, fig.height = 3.5, fig.width = 8, eval = FALSE}
...
ggplot(a, (aes(x = home, y = rank)))+
  geom_text(aes(label = loc_abbr))+
  facet_wrap(vars(energy_cluster), labeller = labeller.all,
             scales = 'free_x')+
  theme_minimal()+
  theme(panel.grid = element_blank())+
  scale_y_discrete(breaks = c('1','2','3'),
...
# to order factors differently in diffrent facets
# https://stackoverflow.com/questions/40744401/change-order-of-factors-within-ggplot-facets



# order of homes for increasing pollutant value
## ac
pm25_ac <- c('012', '009')
voc_ac <- c('012', '009')
co2_ac <- c('012', '009')
...
pm25_all <- c('001', '012', '007', '010', '009', '004', '013', '011', '015')
voc_all <- c('010', '004', '012', '001', '013', '007', '015', '011', '009')
co2_all <- c('012', '007', '004', '013', '010', '001', '015', '011', '009')



rank.line <- function (x_metric){
  
  if(x_metric == 'pm25') metric_title <- expression(paste('PM'[2.5]))
   if(x_metric == 'co2') metric_title <- expression(paste('CO'[2]))
   if(x_metric == 'voc') metric_title <- 'TVOC'
   
  # order metrics based on value (not season specific), depending on metric
  home_order <- case_when(
    x_metric == 'pm25' ~ pm25_all,
    x_metric == 'voc' ~ voc_all,
    x_metric == 'co2' ~ co2_all,
  )
...
a$home <- factor(a$home, levels = home_order)
...
ggplot(a, (aes(x = home, y = rank, group = location)))+
  ...
  theme(
      ...
      legend.text = element_text(size = small-4),
