...
```{r plot_spec_rooms, fig.width = 7, fig.height = 3}
...
#             room_norm <- rnorm(10000, -2.3, 2.3)
#              complement_norm <- rnorm(10000, -2.5, 2.5)
# 
#                           
...
#               room_norm <- rnorm(10000, -2.3, 7)
#              complement1_norm <- rnorm(10000, -2.5, 7.2)
#              complement2_norm <- rnorm(10000, -2.0, 7.1)
...
#             
#             # testing mi.plugin func-------------------------
#             
#             room_norm <- rnorm(100000, -2.3, 7)
#              complement_norm <- rnorm(100000, -2.3, 7)
# 
#              # set up cut-off values 
# breaks <- seq(-2.3, 7, length.out = 101)
# 
...
#                   include.lowest=TRUE, 
#                   right=FALSE)
# room_freqs <- summary(a) %>%as.data.frame() %>%mutate(prob = ./sum(.))
#              
# a <- cut(complement_norm, 
#                   breaks=breaks, 
#                   include.lowest=TRUE, 
#                   right=FALSE)
# complement_freqs <- summary(a) %>%as.data.frame()%>%mutate(prob = ./sum(.))
# 
#              
#              # calculate from entropy R package
#             mut_info1 <- entropy::mi.plugin(cbind(discretize(room_norm, nbins = 30),
#                                             discretize(complement_norm, nbins = 30)))
#              # calculate from infotheo package
#             mut_info1b <- mutinformation(discretize(room_norm, nbins = 30),
#                                         discretize(complement_norm, nbins = 30))
#             
#              #estimate using Osses Normal formula
#              variance_room <- var(room_norm)
