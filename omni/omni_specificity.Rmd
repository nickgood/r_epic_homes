...
```{r import_specificity_data}
spec_df <- read_csv('./csv_created/representativeness_data/specificity.csv')
all.equal(spec_df, spec_df_old)
```
...
spec_df_clean <- spec_df%>% 
  filter(period_compare == 'year', # only comparing sample to whole mon. period
         !is.na(specificity)) # with insufficient data in cluster/year/no cluster
...
scaled_spec_df <- spec_df_clean %>%
  group_by(home) %>%
  mutate(specificity_max = max(specificity)) %>%
  ungroup() %>%
  mutate(scaled_specificity = 1-specificity/specificity_max)


unwanted_scaled_na <- scaled_spec_df %>%
  filter(scaled_specificity %>% is.na())
...
```{r plot_spec_rooms}
ggplot(scaled_spec_df, aes(x = location, y = scaled_specificity))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter( aes(color = energy_cluster))
```

# Testing
* NOTE: PROBABLY OLD FUNCTION

```{r testing_function_specificty_master, eval = FALSE}
