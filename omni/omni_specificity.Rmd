  ...
  scaled.entropy.season <- function(monitor_season) {
    ...
    missing_indeces <- lapply(rooms_with_data, function(x) {
      ...
      unlist()
        ...
        data_wide_complete <- data_wide %>% slice(-c(missing_indeces))
...
#NOTE: if a value is missing from one data set, the function will delete that value from all data-sets when calculating variance and covariance
        ...
        covariance_matrix <- cov(
          matrix(c(data_wide_complete %>% pull(rooms_with_data[1]),
                   data_wide_complete %>% pull(rooms_with_data[2]),
                   data_wide_complete %>% pull(rooms_with_data[3])), ncol = 3), 
          use = 'complete.obs')
        
        cov_determinant <- det(covariance_matrix)
        
        
        variance_room <- var(data_wide_complete %>% pull(room))
        variance_complement <- var(c(data_wide_complete %>% pull(other_rooms[1]),
                                     data_wide_complete %>% pull(other_rooms[2])
        ))
        specificity <-
          -0.5*log( cov_determinant/ (variance_room*variance_complement) )
        
        # using two data sets (room and complement)---------------
        data_comp <- data_wide_complete %>% select(all_of(other_rooms))%>%
          mutate(mean = rowMeans(.)) %>% pull(mean)
        
        covariance_matrix2 <- cov(
          matrix(c(data_wide_complete %>% pull(rooms_with_data[1]),
                   data_comp), ncol = 2), 
          use = 'complete.obs')
        
        cov_determinant2 <- det(covariance_matrix2)
