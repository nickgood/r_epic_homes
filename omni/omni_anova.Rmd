...
map(homes_all, function(x) qqplot.homes(hm = x, met = 'pm25',
                                        log.trans = TRUE))

map(homes_all, function(x) qqplot.homes(hm = x, met = 'voc',
                                        log.trans = TRUE))

map(homes_all, function(x) qqplot.homes(hm = x, met = 'co2',
                                        log.trans = TRUE))
...
levene.test.home <- function(data = omni_avg_data, hm, met, log.trans = FALSE){
...
# test function
levene.test.home(hm = '001', met = 'pm25', log.trans = TRUE)

# test variances for all homes/locations/metrics
map(homes_all, function(x) levene.test.home(hm = x, met = 'pm25',
                                        log.trans = TRUE)) %>%
  bind_rows

map(homes_all, function(x) levene.test.home(hm = x, met = 'voc',
                                        log.trans = TRUE)) %>%
  bind_rows

map(homes_all, function(x) levene.test.home(hm = x, met = 'co2',
                                        log.trans = TRUE)) %>%
  bind_rows
