...
```{r plot_assumption_normality, fig.width=10.5, fig.height=8, eval = FALSE}
...
```{r df_assumption_variances, eval = FALSE}
...
```{r function_anova_homemade, eval = FALSE}
# hm <- '001'
# met <- 'pm25'
# log.trans <- TRUE
sum.square<- function(data = omni_avg_data, hm, met,
  ...
  # and that location's mean
    summarise_at(vars(mean), list(ss.residual = function(x) {sum((x-mean(x))^2)})) %>%
    ungroup() %>%
    t() %>% # transpose
     as.data.frame()
   ...
   k <- ncol(a) # define k based on how many rooms have data

   a[-1,] %>%
     rename_all(function(x) paste0('residual_',x))%>%
     mutate_all(as.numeric) %>%
    mutate(ss_residual = rowSums(.), df_residual = grand_n - k,
           msr = ss_residual/df_residual,
           ss_between = ss_between, df_between = k-1,
           msb = ss_between/df_between,
           f_stat = msb/msr,
           f_crit = qf(p=.05, df1=df_between, df2=df_residual, lower.tail=FALSE),
           null_result = ifelse(f_stat>f_crit, 'reject', 'accept'),
           home = hm, metric = met)
...
```{r df_anova_homemade, eval = FALSE}
...
# using aov and anova (not Anova)----------------------------------
r_anova <- map(metrics_all, function(x_metric) {
  
  map(homes_all, function(x_home) {
    df <- omni_avg_data %>% filter(home == x_home, metric == x_metric,
                                    location %in% locations_indoor,
                                       !is.na(mean))
model <- aov(log(mean)~location, df)
a <- anova(model) 

list(f_stat = a[['F value']][1],
     p_val = a[['Pr(>F)']][1],
     null_result = ifelse(a[['Pr(>F)']][1]<0.05, 'reject', 'accept'),
     home = x_home, metric = x_metric)
...
# # compare to homemade func
# test <- full_join(ss_df2 %>% select(f_stat, null_result,home, metric), r_anova,
#                   by = c('home', 'metric'))%>%
#   mutate(ifelse(null_result.x == null_result.y, TRUE, FALSE))


# # test with Anova func # notice type =3 makes much diff results--------------
# 
# r_anova <- map(metrics_all, function(x_metric) {
#   
#   map(homes_all, function(x_home) {
