...
qqplotter <- function(hm, met,  seas, log.trans = TRUE, resolution = 'hour', 
                      censored = FALSE){
  ...
  data <- data %>%
      select(home, location, all_of(met), season) %>%
...
# filter only data from given season
  ...
  if(nrow(data)==0) NULL else{
  
# Create a QQ plot of values by location
ggqqplot(data, 'value', facet.by = 'location',
                   title = paste('Home', hm, met, seas,
                                 if(log.trans == TRUE) {
                     'log-transformed'} else 'raw data')
)}
}

# # test for one home, location, season
# qqplotter('009', 'pm25', 'ac')
```
...
plots <- map(homes_no5, function(x_home){
        qqplotter(x_home, x_metric, x_season)
      })
plot.grid(plots, paste(x_metric, x_season), ncol = 3)
...
met <- 'pm25'
 ...
 dist_compare_df <- map(clusters_all, function(x_season){
 
   map(homes_no5, function (x_home){
     distribution.compare(x_home, met, x_season)
  }) %>% bind_rows()
     }) %>% bind_rows() %>%
  mutate( condition = paste(location, home, sep = '_')) %>%
