...
```{r fig_total_kwh}
ggplot(df, aes(x = filter_id, y = ir, color = rep)) +
  geom_point() +
  theme_bw()
```
...
For a `r duration_days` day sample, at `r flow_lpm` L/min, onto a `r filter_diam_mm` filter, the effective blank-to-blank variabilty in BC is `r round(d_bc$bc_conc[1], 1)` $\mu g/m^{-3}$, based on the maximum and minimum IR transmission. 
