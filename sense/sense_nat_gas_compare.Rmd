...
```{r}
data <- read_rds('./csv_created_sense/sense_gas_comparison_hourly.rds')
```
...
* check when sense data was available for each home
```{r}
data_dates <- data %>%
  rename(home = job_id) %>%
  filter(home %in% homes_avail,
         datetime < as.Date('2021-06-01')) %>%
  group_by(home) %>%
  summarise(date = unique(datetime), .groups = 'drop') %>%
  mutate(value = 1)


date_list <- map( c('2020-07-01', '2020-08-01', '2020-09-01', '2020-10-01',
...
ggplot(data_dates)+
  ...
  # )
  scale_x_date(
    breaks =  do.call('c', date_list),
    labels = c('J', 'A', 'S', 'O', 'N', 'D', 'J', 'F', 'M', 'A', 'M', 'J')
  )
...
compare_data_wide <- map(homes_avail, bill.compare) %>%
  ...
  left_join(nat_gas_data, by = c("home", "bill_date")) %>%
    filter(end_period < as.Date('2021-06-01')) %>%
  # filter out instances where start/end dates don't extend to
  # start/end billing periods
  left_join(
     data %>%
      rename(home = job_id) %>%
      filter( home %in% homes_avail) %>%
...
ggplot(a) +
  ...
  scale_color_manual(name = "",
                     values = c("Gas Used for Heating" = cbbPalette['light_orange'] %>% unname())) +
  theme_classic()+
  ylab('Normalized Value')+
  xlab('Billing Period Month')

```

* run correlations between nromalized values of heating electric usage from sense
* and nat gas usage
```{r}
cor(compare_data_wide$norm_pct, compare_data_wide$norm_gas, method = 'spearman')
