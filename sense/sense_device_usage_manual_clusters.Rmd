...
type_labs <- c("Tier 1", "Tier 2", "Tier 3")
...
first_date <- daily_energy_data_clustered %>%
  filter(home == x_home, dateday == min(dateday))%>%
  pull(dateday) %>% as.character()

first_date <- ifelse(is_empty(first_date), NA, first_date)

shoulder_date <- season_end_points %>% 
  filter(home == x_home) %>%
  pull(shoulder_start) %>% as.character()

shoulder_date <- ifelse(is_empty(shoulder_date), NA, shoulder_date)

heat_date <- season_end_points %>% 
  filter(home == x_home) %>%
  pull(heat_start) %>% as.character()

heat_date <- ifelse(is_empty(heat_date), NA, heat_date)


last_date <- daily_energy_data_clustered %>%
  filter(home == x_home, dateday == max(dateday))%>%
  pull(dateday) %>% as.character()
...
tibble(home = rep(x_home, 3),
         ...
         start_date = c(first_date,
                        shoulder_date,
                        heat_date),
         end_date = c(shoulder_date,
                        heat_date,
                        last_date))
