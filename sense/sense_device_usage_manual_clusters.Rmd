...
data <- (
  if(resolution == 'hour'){
  read_rds("../output_thesis/sense/sense_hourly_thesis.rds")} else if(resolution == 'minute'){
  read_rds("../output_thesis/sense/sense_all_thesis.rds")}
  )%>%
...
cluster.manual <- function(home_id, devices,
                           include_zero, # if TRUE, include zero in calculation of quartile for daily heat-use
...
units <- 'mins'
  ...
  #   a %>% pull(heat) %>% quantile(heat_percentile_thresh)
  # } else if(include_zero == FALSE){
  #         a %>% filter(heat >0) %>% pull(heat)%>%
  #     quantile(heat_percentile_thresh)
  #   }
    a %>%
    #find month with max avg daily heat pct on
    # and make threshold 15% of that value
    group_by(month = month(dateday, label = TRUE, abbr = TRUE)) %>%
...
# # test function
# test <- cluster.manual('001', c('heat 6','ac'), return_data = TRUE, include_zero = zero_choice)
...
cluster_df_001 <- cluster.manual(hm, c('heat 6','ac'), return_data = TRUE,
                                 include_zero = zero_choice)


hm <- '002'
#
cluster_df_002 <- cluster.manual(hm, c('furnace','ac 2'), return_data = TRUE, include_zero = zero_choice)


hm <- '007'
# 
cluster_df_007 <- cluster.manual(hm, c('heat 2','ac'), return_data = TRUE, include_zero = zero_choice)


hm <- '009'
# many heats that could be feasible...
# consider summing consumption from heat 2 and 3 with furnace to cluster
cluster_df_009 <- cluster.manual(hm, c('furnace','ac'), return_data = TRUE, include_zero = zero_choice)



hm <- '012'
#
cluster_df_012 <- cluster.manual(hm, c('furnace','ac'), return_data = TRUE, include_zero = zero_choice)

# tier 2 homes

hm <- '004'
# no AC
cluster_df_004 <- cluster.manual.heat(hm, c('furnace'), return_data = TRUE, include_zero = zero_choice)



hm <- '010'
# no ac (later enrollment), also no one heat that makes complete sense by itself...
cluster_df_010 <- cluster.manual.heat(hm, c('heat 2'), return_data = TRUE, include_zero = zero_choice)


hm <- '011'
# no ac (later enrollment)
cluster_df_011 <- cluster.manual.heat(hm, c('heat 6'), return_data = TRUE, include_zero = zero_choice)
...
# only 4 days of ac detected, so not used to cluster
# no heats consume much, but heat 2 is on most often/constently
cluster_df_013 <- cluster.manual.heat(hm, c('heat 2'), return_data = TRUE, include_zero = zero_choice)



hm <- '015'
# no ac, multiple feasible heats
cluster_df_015 <- cluster.manual.heat(hm, c('furnace'), return_data = TRUE, include_zero = zero_choice)
...
hm <- '006'
#
cluster_df_006 <- cluster.manual(hm, c('heat 1','ac'), return_data = TRUE, include_zero = zero_choice)
...
# many heats that could be feasible...
# heat 1 and 2 on approx same fraction, but 2 consumes more energy
cluster_df_008 <- cluster.manual(hm, c('heat 1','ac'), return_data = TRUE, include_zero = zero_choice)
...
# heat doesn't look feasible...
# ac on at unexpected months
cluster_df_014 <- cluster.manual(hm, c('heat 1','ac'), return_data = TRUE, include_zero = zero_choice)
...
write_csv(daily_energy_data_clustered %>%
            mutate(dateday = as.character(dateday)), paste0('./csv_created_sense/daily_energy_data_clustered_', Sys.Date(),'.csv')
)
...
# a$dateday[sum(y$lengths[1:final_heat_index])+1]
# # need to add condition if NA (if the last heat period is the end,
# # so nothing occurs after the last index)
# 
# 
# # return position of end of 8-day stretch
...
write_csv(season_pcts, './csv_created_sense/season_pcts.csv')
```
