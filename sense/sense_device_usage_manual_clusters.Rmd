  ...
  a <- data %>%
    ...
    mutate(
         ...
         pct = case_when(unit_per_day < unit_check ~ 0,
                         TRUE ~ pct)) %>% # make cluster of time periods
    select(-c(on_per_day,unit_per_day))%>%
...
# many heats that could be feasible...
# heat 1 and 2 on approx same fraction, but 2 consumes more energy
cluster_df_008 <- cluster.manual(hm, c('heat 2','ac'), return_data = TRUE, include_zero = zero_choice)
