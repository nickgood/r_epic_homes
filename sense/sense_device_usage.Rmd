...
```{r function_daily_fraction, include = FALSE, eval = FALSE}
...
```{r functions_daily_consum, include = FALSE, eval = FALSE}
...
# Cluster days based on heating and ac behaviours using PAM 


```{r functions_pam_cluster, include = FALSE, eval = FALSE}
...
```{r pam_cluster_df_maker, eval = FALSE}
...
```{r plot_pam_cluster_fraction, eval = FALSE, fig.width=10, fig.height=3}
...
```{r plot_pam_cluster_consum, eval = FALSE, fig.width=10, fig.height=3}
...
```{r pam_cluster_df_maker_consum_summed, eval = FALSE}
...
```{r plot_pam_cluster_consum_summed, eval = FALSE, fig.width=10, fig.height=3}
...
```{r function_cluster_compare, eval = FALSE}
...
# perform PAM Clustering with euclidean distance
  pam_calc <- pam(a %>% select(c(heat_scaled, ac_scaled)), n_clust)
  
# perform hierarchical clustering
dist_mat <- dist(a %>% select(-c(dateday, heat, ac)))

# make cuts from different hierarchical methods
cut_avg <- cutree(hclust(dist_mat, method = 'average' ), k = n_clust)
cut_sing <- cutree(hclust(dist_mat, method = 'single'), k = n_clust)
cut_ward <- cutree(hclust(dist_mat, method = 'ward.D' ), k = n_clust)
cut_ward2 <- cutree(hclust(dist_mat, method = 'ward.D2'), k = n_clust)
cut_comp <- cutree(hclust(dist_mat, method = 'complete' ), k = n_clust)
cut_mcq <- cutree(hclust(dist_mat, method = 'mcquitty'), k = n_clust)
cut_cent <- cutree(hclust(dist_mat, method = 'centroid'), k = n_clust)
...
a_cl <- a %>%
  ...
  mutate(
    # pam
    cluster_pam = pam_calc$clustering,
    # hierarchical
           cluster_avg = cut_avg, cluster_sing = cut_sing, cluster_ward = cut_ward,
         cluster_ward2 = cut_ward2, cluster_comp = cut_comp, cluster_mcq = cut_mcq,
         cluster_cent = cut_cent) %>%
  pivot_longer(cols = c(cluster_pam,
                        cluster_avg, cluster_sing, cluster_ward, cluster_ward2,
...
```{r plots_cluster_compare, eval = FALSE, fig.width=10, fig.height=5}
...
energy_cluster_df<- tribble(~home, ~cluster_type,
                     ...
                     '011', 'shoulder', '2020-08-01', '2020-12-01',
                     ...
                     '013', 'shoulder', '2020-08-01', '2020-11-07',
                     '013', 'heat', '2020-11-08', '2021-03-31',
                     '013', 'shoulder_2', '2021-04-01', '2021-05-23',
                     '015', 'shoulder', '2020-08-01', '2020-11-23',
...
#create csv
write_csv(energy_cluster_df, './csv_created_sense/home_type_df.csv')
