...
# classify hourly data as night data or day data
```{r}
...
# stratify by hour type (day/night)
# all metrics at one time
...
ggplot(a, aes(x = living)) +
  # geom_point(aes( y = kitchen), color = room_colors[3])+
  # geom_raster( aes(y = kitchen, fill = after_stat(density)))
stat_bin2d(aes(y = kitchen,  fill = after_stat(count)), binwidth = c(5,5))+
  facet_grid(rows = vars(time), cols = vars(metric), scales = 'free')+
  theme_classic()
...
cor_df <- lapply(list( c('kitchen', 'day'),
             c('kitchen', 'night'),
             c('bedroom', 'day'),
             c('bedroom', 'night')),
       function(x) {
         
         data <- a %>% filter(location == x[1], time == x[2])
...
# stat_bin2d(aes(y = value,  fill = after_stat(count)), binwidth = c(5,5))+
  ...
  geom_text( data = cor_df,  aes(label = rho_label ),
             parse = T,
             x = 0.75*max, y = 0.25*max)+
    geom_text( data = cor_df,  aes(label = paste0('n=', n_cor) ),
             x = 0.75*max, y = 0.17*max)+
  scale_color_manual(
...
lapply(c( 'pm25', 'voc', 'co2'), function(x) correl.plot(x) )
```
