...
```{r import_data}
# hourly omni data
omni_hourly<- read_rds('./csv_created_article/omni_hourly_calibrated.rds')%>% filter(datehour<ymd_hms('2021-06-01 00:00:00')) # only use data before june 6

# home groups
home_type_df <- read_csv('../sense/csv_created_sense_article/home_type_df.csv')

# energy cluster dataframe for all homes
...
lag_summary_df <- read_csv('./csv_created_article/acf/lag_summary.csv')
      ...
      a <-
        runner(
          ...
          # number of hour periods in short sampling period
               ...
               at = seq(days*24, length(all_dates_season), days*24),
...
# count omissions from Group 3 Homes
# (only group 3 homes are compared later in Representativeness analysis)


```{r}
hm <- '009'

# all data for group 3 homes
a <-omni_hourly %>%
  filter( location %in% locations_indoor & home %in% homes_tier3)


# count Group 3 home data that did not fit into a continuous season
# because of lacking sense data at the end or beginning of sampling campaign,
# or after because of excluding data after the end of the heating season
# note: Home 7 wifi caused 23.7% of data loss for that home

c <- 
  lapply(homes_tier3,  function(x_home) {
    
home_data <- a %>% filter(home == x_home)

    date_start <- pick.start(x_home, 'ac')
    date_end <- pick.end(x_home, 'heat')

data_omitted <- home_data %>%
  filter( datehour > ymd(date_end) | datehour < ymd(date_start))

# count rows of data omitted
list( 'home' = x_home, 'omitted' = nrow(data_omitted),
      'frac_omitted' = nrow(data_omitted) / nrow(home_data)
)
  }
) %>% bind_rows() %>%
  # what fraction of total amount omitted did each home contribute
  mutate(total_frac = omitted/ sum(omitted))


# fraction of Group 2 and 3 Home Data omitted
c %>%  pull(omitted) %>% sum()  /    nrow(a)

```

### Make Representativeness Dataframes/csvs
...
lapply(clust, function(cluster_type) {
  ...
  rep_data <-  lapply(homes_seasons, function(x_home) {
    ...
    entropy.table.hourly(
      ...
      date_ranges_entire = list(c(start_date,
                                  end_date)),
      tested_sample_sequence = c(1, 3, 7, 14),
      location_type = loc
      ) %>%
...
# ) %>%
#   write_rds ('./csv_created_article/representativeness_data/test_after_edit.rds')
#  
# test <- read_rds('./csv_created_article/representativeness_data/test_before_edit.rds')
# 
# test_after <- read_rds('./csv_created_article/representativeness_data/test_after_edit.rds')
# 
# all.equal(test, test_after)
...
#             paste0('./csv_created_article/representativeness_data/rep_data_time.rds'))
# # make dated backup
# write_rds(entropy_time_hourly_pdf_df, file =
#             paste0('./csv_created_article/representativeness_data/rep_data_time_',
#                    Sys.Date(),
#                    '.rds'))

# for joining files ---------------------

# rep_co2 <- read_rds('./csv_created_article/representativeness_data/co2_rep_data_time.rds')
# 
# rep_voc <- read_rds('./csv_created_article/representativeness_data/voc_rep_data_time.rds')
# 
# rep_pm25 <- read_rds('./csv_created_article/representativeness_data/pm25_rep_data_time.rds')
# 
# entropy_time_hourly_pdf_df_all <- bind_rows(
#   rep_co2, rep_voc, rep_pm25
# )
# 
# 
# write_rds(entropy_time_hourly_pdf_df_all, file =
#             paste0('./csv_created_article/representativeness_data/rep_data_time.rds'))
# # make dated backup
# write_rds(entropy_time_hourly_pdf_df_all, file =
#             paste0('./csv_created_article/representativeness_data/rep_data_time_',
