...
entropy_df <- 
    ...
    bind_rows(
      ...
      read_rds(file = './csv_created_article/representativeness_data/rep_data_time.rds')
...
entropy_df_clean <- entropy_df%>% 
  ...
  mutate(
                ...
                outlier = 
                     case_when(  kld > quantile(kld, 0.75)+
                    1.5*IQR(kld)~ TRUE,
                    TRUE ~FALSE),
            ext_outlier = 
                     case_when(  kld > quantile(kld, 0.75)+
    ...
    # mistake made during thesis, kept here to allow for comparison
            # outlier = 
...
## Example hour-of-day/Representativeness Comparison Plot
...
* plotting function
* use systemartic method to select sample data for plotting
```{r}
...
diurn.rep.plot <- function(data, x_home, x_location, x_energy_cluster, x_metric,
         sample_lengths = c(1,3,7), # has to be a numerical vector of length 3 or 4
         y_limits = NULL){
...
# https://stackoverflow.com/questions/42891307/how-can-i-maintain-a-color-scheme-across-ggplots-while-dropping-unused-levels-i
  ...
  ggplot(all_data, aes(x = hour, y = mean,
    ...
    theme(
      ...
      legend.text = element_text(size = medium-5),
            legend.title = element_text(size = medium-5),
      axis.text = element_text(size = medium - 6),
...
* Plot for specified home, season, and room
...
* faceted by season
* see plot with pooled seasons in past code versions
...
a <- entropy_time %>% filter(sample_length %in% c(1,3,7) &
                               !is.na(rep) &
                               metric == x_metric & 
                               home %in% homes_tier3)
...
a <- a %>%
  group_by(energy_cluster, sample_length) %>%
    mutate(n = n(),
         ...
         label = paste0(sample_length, ",  n=",as.integer(n),",  ", above1, "%")
)
...
p1 <- ggplot(a[['ac']])+
  ...
  geom_vline(xintercept = thresh_vector[1], linetype = 'dashed')+
...
p1 <- ggplot(a[['ac']])+
  ...
  ggtitle(paste0(x_metric, ': Samples from GROUP 3 homes and rooms pooled'))+
    ...
    theme(
      ...
      strip.text = element_text(size = medium),
      axis.text = element_text(size = medium-4),
      axis.title.x = element_text(size = medium-3),
      axis.title.y = element_text(size = medium+2),
      legend.text = element_text(size = medium+2),
